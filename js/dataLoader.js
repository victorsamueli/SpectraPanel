...
const DataLoader = {
            ...
            reader.onload = (e) => {
                try {
                    ...
                    workbook.SheetNames.forEach(sheetName => {
                        ...
                        // Normalize whitespace, strip trailing asterisk/indicators from keys, and assign ID if missing
                        const cleanJson = [];
    ...
    processSingleTableData(data, filename) {
        ...
        // Normalize keys (strip * and whitespace) and assign ID
        const cleanData = [];
        data.forEach((row, idx) => {
            ...
            if (!cleanRow.id) cleanRow.id = `item_${idx + 1}`;
    ...
    clearDatabase(confirmWithUser = true) {
        ...
        if (ui) {
            if (ui.state) {
                ui.state.selectedReagents = [];
            }
            if (typeof ui.renderSelectedTargetsBox === 'function') {
                ui.renderSelectedTargetsBox();
            }
            if (typeof ui.clearResults === 'function') {
                ui.clearResults();
            }
        }
        document.dispatchEvent(new CustomEvent('dbLoaded'));
    ...
    generateExcelTemplate() {
        ...
        const instructionsData = [
            ...
            ["2. Columns marked with an asterisk (*) are essential for SpectraPanel combination design & channel matching."],
            ["   Essential columns:"],
            ["     • Primaries: target*, clonality*, host*, isotype*, fixation_compatible*"],
            ["     • Secondaries: anti_host*, conjugate*, emission_nm*"],
            ["     • Direct_Dyes: name*, emission_nm*, live_cell_compatible*"],
            ["     • Reporters: target*, reporter*, emission_nm*"],
            ["   Other columns (such as clone, make, catalog, excitation_nm, comments) are optional for laboratory tracking."],
            ["3. Detection channels & colors for Secondaries, Dyes, and Reporters are computed dynamically from emission peaks matching your configured microscopy channels."],
            ["4. Under Reporters, enter target* (protein name) and reporter* (fluorophore tag / FP)."],
            ...
            ["• Primaries Sheet:"],
            ["    - target*: Target protein or marker (e.g., 'Ki67', 'alpha-Tubulin') [Essential]"],
            ["    - clonality*: 'Monoclonal', 'Polyclonal', or 'Recombinant Monoclonal' [Essential]"],
            ["    - clone: Clone name or ID (e.g., 'SP6', 'DM1A')"],
            ["    - host*: Primary host species (e.g., 'Mouse', 'Rabbit') [Essential]"],
            ["    - isotype*: Primary isotype (e.g., 'IgG1', 'IgG2a', 'IgG2b', 'IgG', 'IgM') [Essential]"],
            ...
            ["    - fixation_compatible*: 'PFA', 'Methanol', or 'PFA, Methanol' [Essential]"],
            ["    - live_cell_compatible: 'Yes' or 'No'"],
            ["    - comments: Lab notes, storage box, aliquot notes, etc."],
            [""],
            ["• Secondaries Sheet:"],
            ["    - anti_host*: Target species recognized by the secondary (e.g., 'Mouse', 'Rabbit', 'Chicken') [Essential]"],
            ...
            ["    - conjugate*: Fluorophore or enzyme name (e.g., 'Alexa Fluor 488', 'Alexa Fluor Plus 647', 'HRP') [Essential]"],
            ["    - conjugate_type: 'Fluorophore' or 'HRP'"],
            ["    - excitation_nm: Peak excitation wavelength in nm (e.g., 490, 650)"],
            ["    - emission_nm*: Peak emission wavelength in nm (e.g., 525, 665) [Essential]"],
            ...
            [""],
            ["• Direct Dyes Sheet:"],
            ["    - name*: Dye name (e.g., 'DAPI', 'Phalloidin') [Essential]"],
            ["    - target_structure: Cellular target (e.g., 'DNA', 'F-Actin')"],
            ["    - color: General color description (e.g., 'Blue', 'Green', 'Far-Red')"],
            ["    - excitation_nm: Peak excitation in nm (e.g., 360, 495)"],
            ["    - emission_nm*: Peak emission in nm (e.g., 460, 518) [Essential]"],
            ["    - live_cell_compatible*: 'Yes' or 'No' [Essential]"],
            ["    - make: Manufacturer / Supplier"],
            ["    - catalogue: Product catalog number"],
            [""],
            ["• Reporters Sheet:"],
            ["    - target*: Target protein name (e.g., 'Tubulin', 'Actin') [Essential]"],
            ["    - reporter*: Fluorophore tag / FP name (e.g., 'EGFP', 'mCherry') [Essential]"],
            ["    - excitation_nm: Peak excitation in nm (e.g., 488, 587)"],
            ["    - emission_nm*: Peak emission in nm (e.g., 507, 610) [Essential]"],
            ["    - recommended_fixation: Compatible fixation conditions (e.g., '2% PFA 10 min RT')"],
            ["    - live_cell_compatible: 'Yes' or 'No'"]
        ];
